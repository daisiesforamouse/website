...
Furthermore, the higher homotopy groups must be 0, since we have that the fiber bundle \(O_n \rightarrow  O_{n-1}\) via forgetting the last coordinate has fiber \(\C \setminus \{z_1, \dots, z_n-1\}\), and so we must have that for \(k \geq 2\), we have the l.e.s.
\[
...
#+END_EXPORT
But we can just check this for \(S_{ij}\) the commutator of \(\sigma_i, \sigma_j\), and we win.

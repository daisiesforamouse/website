...
1. If each \(Z_i\) has the "correct" dimension, i.e. \(\dim(Z_i) = \dim(V) + \dim(W) - n > 0\), then there exist integers \(m_i \geq 1\) such that \(\sum_{i=1}^r m_i \deg(Z_i) = \deg(V) \cdot \deg(W)\).
...
# Let \(S = \Spec(R)\) and \(G = \Spec(A)\) with some ring map \(R \rightarrow A\). Then we get some
# \begin{gather*}
#     \mu: A \rightarrow   A \otimes_R A \text{ comultiplication} \leftrightarrow m: G \times_S G \rightarrow  G \\
#     e: A \rightarrow   R \text{ augmentation} \leftrightarrow e: S \rightarrow  G \\
#     i: A \rightarrow   A \text{ inverse} \leftrightarrow i: G \rightarrow  G \\
# \end{gather*}
# #+BEGIN_EXPORT html
# <p align="center">
# <img src="ag12.png" style="width:40%">
# </p>
# #+END_EXPORT
...
1. And if yes, when can we pick \(\alpha\) such that the cocycle condition holds?
